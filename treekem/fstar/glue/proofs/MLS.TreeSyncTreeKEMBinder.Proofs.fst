...
val pathsync_pre_pathkem_rel_to_pathsync_pathkem_rel:
  #bytes:Type0 -> {|crypto_bytes bytes|} ->
  #l:nat -> #i:tree_index l -> #li:leaf_index l i ->
  ps:pathsync bytes tkt l i li -> p:path_secrets bytes l i li -> ppk:pre_pathkem bytes l i li -> pk:pathkem bytes l i li ->
  ...
  Lemma
  (requires
    Success ppk == TK_ops.path_secrets_to_pre_pathkem p /\
    (exists priv_st. Success (pk, priv_st) == TK_ops.encrypt_path_secrets tk p group_context rand) /\
    pathsync_pre_pathkem_rel ps ppk
  )
  (ensures pathsync_pathkem_rel ps pk)
let rec pathsync_pre_pathkem_rel_to_pathsync_pathkem_rel #bytes #cb #l #i #li ps p ppk pk tk group_context rand =
  let open TK_ops in
  if l = 0 then ()
  else (
    ...
    pathsync_pre_pathkem_rel_to_pathsync_pathkem_rel (PNode?.next ps) (PNode?.next p) (PNode?.next ppk) (PNode?.next pk) child group_context rand_next
  )
#pop-options
...
val create_commit_state_rel:
  ...
  st_tk:treekem_tree_state bytes leaf_ind -> rand_prepare:randomness bytes (TK_API.prepare_create_commit_entropy_lengths bytes) ->
  tk_pend:TK_API.pending_commit st_tk -> pre_path:pre_pathkem bytes st_tk.levels 0 leaf_ind ->
  ...
  (requires
    li_ts == leaf_ind /\
    st_ts `treesync_treekem_state_rel` st_tk /\
    TK_API.prepare_create_commit st_tk rand_prepare == Success (tk_pend, pre_path) /\
    // the following assertion is proved using `pathsync_pre_pathkem_rel_external_path_to_path`
    p_ts `pathsync_pre_pathkem_rel` pre_path
  )
  (ensures (
...
let create_commit_state_rel #bytes #cb #token_t #group_id #leaf_ind #st_ts #li_st #p_ts ts_pend token st_tk rand_prepare tk_pend pre_path added_leaves group_context rand_finalize =
  match TS_API.finalize_commit ts_pend token, TK_API.finalize_create_commit tk_pend added_leaves group_context rand_finalize with
  | Success res_ts, Success res_tk -> (
    ...
    pathsync_pre_pathkem_rel_to_pathsync_pathkem_rel p_ts tk_pend.path_secrets pre_path res_tk.TK_API.update_path (TK_ops.un_add st_tk.tree added_leaves) group_context rand_finalize;
