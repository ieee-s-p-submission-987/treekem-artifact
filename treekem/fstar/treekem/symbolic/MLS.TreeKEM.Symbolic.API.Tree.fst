...
#push-options "--fuel 1 --ifuel 1"
val get_path_leaf_path_secrets_to_pre_pathkem:
  #l:nat -> #i:tree_index l -> #li:leaf_index l i ->
  p:path_secrets bytes l i li ->
  Lemma (
    match path_secrets_to_pre_pathkem p with
    | Success res -> (get_path_leaf res).public_key == hpke_pk (get_path_leaf p)
    | _ -> True
  )
let rec get_path_leaf_path_secrets_to_pre_pathkem #l #i #li p =
  match p with
  | PLeaf _ -> ()
  | PNode _ p_next -> get_path_leaf_path_secrets_to_pre_pathkem p_next
#pop-options
    ...
    | Success (pend, pre_path) -> (
      ...
      | Success external_path -> (
        (get_path_leaf external_path).source == LNS_update /\ (
          if Success? (external_path_to_path_nosig st_ts.tree external_path) && leaf_ind < pow2 32 then
...
val create_commit_proof:
  ...
  st_ts:treesync_state bytes tkt dy_as_token group_id -> st_tk:treekem_tree_state bytes leaf_ind{st_ts.levels == st_tk.levels} ->
  rand_prepare:randomness bytes (prepare_create_commit_entropy_lengths bytes) ->
  tk_pend:TK.pending_commit st_tk -> pre_path:pre_pathkem bytes st_tk.levels 0 leaf_ind ->
  ...
  Lemma
  (requires
    pathsync_pre_pathkem_rel auth_path pre_path /\
    TK.prepare_create_commit st_tk rand_prepare == Success (tk_pend, pre_path) /\
...
let create_commit_proof #invs #group_id #leaf_ind tr me st_ts st_tk rand_prepare tk_pend pre_path added_leaves group_context rand_finalize auth_path ts_pend token =
  let leaf_sk, rand_prepare = dest_randomness rand_prepare in
  let path_secret_0, rand_prepare = dest_randomness rand_prepare in
  generate_path_secrets_proof me st_ts.tree st_tk.tree leaf_sk path_secret_0 leaf_ind group_id tr [];
  assert(pre_path = Success?.v (path_secrets_to_pre_pathkem tk_pend.path_secrets));

  assert(path_secrets_are_correct me true st_ts.tree group_id tk_pend.path_secrets tr);
  ...
  | Success ts_res, Success tk_res -> (
    ...
    assert(
      Success ts_res.tree = MLS.TreeSync.Operations.apply_path st_ts.tree auth_path /\
      Success pre_path == path_secrets_to_pre_pathkem tk_pend.path_secrets /\
      Success (tk_res.update_path, tk_res.new_state.priv) == encrypt_path_secrets (un_add st_tk.tree added_leaves) tk_pend.path_secrets group_context rand_finalize
    ) by (let open FStar.Tactics in set_ifuel 1; set_rlimit 50);
    pathsync_pre_pathkem_rel_to_pathsync_pathkem_rel auth_path tk_pend.path_secrets pre_path tk_res.update_path (un_add st_tk.tree added_leaves) group_context rand_finalize;
    assert(pathsync_pathkem_rel auth_path tk_res.update_path);
    apply_path_treesync_treekem_rel st_ts.tree st_tk.tree auth_path tk_res.update_path;
    assert(ts_res.tree `treesync_treekem_rel` tk_res.new_state.TKT.tree);
    get_path_leaf_path_secrets_to_pre_pathkem tk_pend.path_secrets;
    get_path_leaf_pathsync_pre_pathkem_rel auth_path pre_path;
    assert((get_path_leaf auth_path).data.content == (hpke_pk leaf_sk));
