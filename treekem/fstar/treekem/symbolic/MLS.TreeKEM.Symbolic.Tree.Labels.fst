...
[@@ with_bytes bytes]
type leaf_node_has_been_verified = {
...
instance event_leaf_node_has_been_verified: event leaf_node_has_been_verified = {
  tag = "MLS.TreeSync.LeafNodeHasBeenVerified";
  format = mk_parseable_serializeable ps_leaf_node_has_been_verified;
}

val leaf_node_has_been_verified_pred: {|crypto_invariants|} -> event_predicate leaf_node_has_been_verified
let leaf_node_has_been_verified_pred #cinvs =
...
val has_leaf_node_has_been_verified_invariant:
  {|protocol_invariants|} ->
  prop
let has_leaf_node_has_been_verified_invariant #invs =
  has_event_pred leaf_node_has_been_verified_pred

val leaf_node_has_been_verified_tag_and_invariant: {|crypto_invariants|} -> (string & compiled_event_predicate)
let leaf_node_has_been_verified_tag_and_invariant #cinvs =
  (event_leaf_node_has_been_verified.tag, compile_event_pred leaf_node_has_been_verified_pred)

val i_have_verified_leaf_node:
  trace -> principal ->
  leaf_node_nt bytes tkt -> mls_bytes bytes -> nat ->
  prop
let i_have_verified_leaf_node tr me leaf_node group_id leaf_index =
...
//val canonicalize_leaf_signature_unmerged_leaves_commutes_aux:
//  #bytes:Type0 -> {|bytes_like bytes|} -> #tkt:treekem_types bytes ->
//  #l:nat -> #i:tree_index l ->
//  t:treesync bytes tkt l i -> li:leaf_index l i -> p:(nat -> bool) ->
//  Lemma
//  ((un_addP (canonicalize_leaf_signature t li) p == canonicalize_leaf_signature (un_addP t p) li))
//let rec canonicalize_leaf_signature_unmerged_leaves_commutes_aux #bytes #bl #tkt #l #i t li p =
//  match t with
//  | TLeaf _ -> ()
//  | TNode _ left right -> (
//    let (child, sibling) = get_child_sibling t li in
//    canonicalize_leaf_signature_unmerged_leaves_commutes_aux child li p
//  )
//
//val canonicalize_leaf_signature_unmerged_leaves_commutes:
//  #bytes:Type0 -> {|bytes_like bytes|} -> #tkt:treekem_types bytes ->
//  #l:nat -> #i:tree_index l ->
//  t:treesync bytes tkt l i -> li:leaf_index l i ->
//  Lemma
//  ((canonicalize_unmerged_leaves (canonicalize_leaf_signature t li) == canonicalize_leaf_signature (canonicalize_unmerged_leaves t) li))
//let canonicalize_leaf_signature_unmerged_leaves_commutes #bytes #bl #tkt #l #i t li =
//  match t with
//  | TLeaf _ -> ()
//  | TNode None _ _ -> ()
//  | TNode (Some content) _ _ ->
//    canonicalize_leaf_signature_unmerged_leaves_commutes_aux t li (un_add_unmerged_leaf content.unmerged_leaves)
...
val i_have_verified_every_leaf_node:
  ...
  treesync bytes tkt l i -> mls_bytes bytes ->
  prop
let i_have_verified_every_leaf_node #l #i tr me t group_id =
  forall li.
    match leaf_at t li with
    | None -> True
...
let rec prove_all_tree_keys_good #invs #l #i tr me t ast group_id =
  ...
  | TNode opt_pn left right, TNode _ aleft aright -> (
    introduce forall li. one_credential_ok left aleft li with assert(one_credential_ok t ast li);
    introduce forall li. one_credential_ok right aright li with assert(one_credential_ok t ast li);
    introduce forall li. match leaf_at left li with | None -> True | Some ln -> i_have_verified_leaf_node tr me ln group_id li with (let ln = leaf_at t li in ());
    introduce forall li. match leaf_at right li with | None -> True | Some ln -> i_have_verified_leaf_node tr me ln group_id li with (let ln = leaf_at t li in ());
    prove_all_tree_keys_good tr me left aleft group_id;
    prove_all_tree_keys_good tr me right aright group_id;
    match opt_pn with
    | None -> ()
    | Some pn -> all_tree_keys_good_parent_hash_case tr me t ast group_id
  )
#pop-options
