...
val hpke_ciphertext_is_publishable:
  {|crypto_invariants|} ->
  trace -> hpke_ciphertext_nt bytes ->
  prop
let hpke_ciphertext_is_publishable #cinvs tr ciphertext =
  is_publishable tr ciphertext.kem_output /\
  is_publishable tr ciphertext.ciphertext
...
val pathkem_good:
  ...
  trace ->
  prop
let rec pathkem_good #cinvs #l #i #li p tr =
  match p with
  | PLeaf { public_key } ->
    is_publishable tr public_key
  | PNode opt_node p_next ->
    pathkem_good p_next tr /\ (
      match opt_node with
      | None -> True
      | Some node ->
        is_publishable tr node.encryption_key /\
        for_allP (hpke_ciphertext_is_publishable tr) node.encrypted_path_secret
    )

val encrypt_path_secrets_entropy_ghost_data:
  #l:nat -> #i:tree_index l -> #li:leaf_index l i ->
  ts:treesync bytes tkt l i ->
  tk:treekem bytes l i -> path unit bool l i li ->
  mls_bytes bytes ->
  (nat -> bool) ->
  Pure randomness_ghost_data
  (requires
...
val get_path_secret_of_added_leaf_proof:
  ...
  (ensures (
    ...
    | Success path_secret ->
      // TODO: similar to `decoded_path_secret_good`
      bytes_invariant tr path_secret /\
      get_label tr path_secret `can_flow tr` (
        match leaf_at ts leaf_li with
        | Some ln -> node_sk_label_aux_leaf ln group_id YesCommitter leaf_li
        | None -> secret
      ) /\
      (exists data. path_secret `has_usage tr` (KdfExpandKey "MLS.PathSecret" data))
    | _ -> True
...
val pathkem_good_weak:
  ...
  trace ->
  prop
let rec pathkem_good_weak #cinvs #l #i #li p tr =
  match p with
  | PLeaf { public_key } -> True
  | PNode opt_node p_next ->
    pathkem_good_weak p_next tr /\ (
      match opt_node with
      | None -> True
      | Some node ->
        bytes_invariant tr node.encryption_key /\
        (forall ciphertext. List.Tot.memP ciphertext node.encrypted_path_secret ==>
          bytes_invariant tr ciphertext.kem_output /\
          bytes_invariant tr ciphertext.ciphertext
        )
    )
