...
val init_key_is_compromised:
  #l:nat ->
  trace ->
  bytes -> group_context_nt bytes -> treesync bytes tkt l 0 ->
  prop
let init_key_is_compromised #l tr init_secret group_context tree =
  exists leaf_index.
...
val psk_secret_label_corrupt_implies_all_psks_are_publishable:
  ...
  Lemma
  (requires
    bytes_invariant_psks tr psks /\
    is_corrupt tr (compute_psk_secret_label tr psks)
  )
...
val treekem_security_theorem_for_init_secret:
  {|protocol_invariants|} ->
  tr:trace ->
  ...
  (requires
    ...
    is_publishable tr ev.init_secret /\
    trace_invariant tr /\
    has_treekem_invariants
  )
  (ensures
    ...
    (
      init_key_is_compromised tr ev.init_secret ev.group_context ev.tree
    ) \/
    // (2)--(9) see theorem `treekem_security_theorem_for_epoch_secret` below
    (
      is_publishable tr ev.epoch_secret
    )
  )
let treekem_security_theorem_for_init_secret #invs tr me ev =
  reveal_opaque (`%i_am_in_group_pred) (i_am_in_group_pred (prefix tr (find_event_triggered_at_timestamp tr me ev)) me ev);
  assert(is_corrupt tr (join (get_label tr ev.epoch_secret) (mk_epoch_secret_label InitSecret ev.group_context)));
  introduce is_corrupt tr (mk_epoch_secret_label InitSecret ev.group_context) ==> init_key_is_compromised tr ev.init_secret ev.group_context ev.tree with _. (
    mk_epoch_secret_label_eq InitSecret ev.group_context ev.tree;
    mk_tree_epoch_secret_label_is_corrupt tr InitSecret ev.group_context ev.tree
  )

#push-options "--ifuel 1 --z3rlimit 25"
val treekem_security_theorem_for_epoch_secret:
  {|protocol_invariants|} ->
  tr:trace ->
  me:principal -> ev:i_am_in_group ->
  Lemma
...
let treekem_security_theorem_for_epoch_secret #invs tr me ev =
  reveal_opaque (`%i_am_in_group_pred) (i_am_in_group_pred (prefix tr (find_event_triggered_at_timestamp tr me ev)) me ev);
  ...
  | ProcessedCommit last_epoch_link -> (
    ...
    let epoch_secret_label =
      meet
        ...
        psk_secret_label
    in
    assert(is_corrupt tr epoch_secret_label);
    assert(
      (
        is_publishable tr (last_epoch_link.previous_init_secret) /\
    ...
    psk_secret_label_corrupt_implies_all_psks_are_publishable tr ev.psks;
    introduce
      is_corrupt tr joiners_label ==>
      exists joiner.
        List.Tot.memP joiner last_epoch_link.joiners /\ (
